...
| Confirmations | controlled title, description, confirm label, pending/tone state, and one optional short text input; no custom content or styling overrides                                                                                        |
| Action menus  | fixed Manage trigger; stable item ID, label, semantic tone, disabled state, and selection callback; no custom trigger or menu content                                                                                              |
| Toasts        | `Toaster` root and `notify({ type, title, description, duration?, closeAutomatically? })`; `notify.promise` supports loading, success, and error states through the current shadcn/Base UI Toast implementation only, never Sonner |
...
| ./components/table             | Table and semantic table parts | `Table`: `children`, `ariaLabel`, `overflowHint`, `surface`, `data-testid`; parts accept only their documented structural and named presentation decisions |
| ./components/toolbar           | Toolbar          | `ariaLabel`, `open`, `summary`, `items`, `data-testid`                                                                                                                                                                                                              |
...
| ./components/alert-dialog      | AlertDialog      | `open`, `onOpenChange`, `title`, `description`, `confirmLabel`, `onConfirm`, `pending`, `tone`, `input`                                                                                                                                                             |
| ./components/action-menu       | ActionMenu       | `label`, `items`                                                                                                                                                                                                                                                    |
| ./components/toast             | Toaster, notify  | `children`; `type`, `title`, `description`, `duration`, `closeAutomatically`; `notify.promise`                                                                                                                                                                      |

Public supporting types: `ButtonProps`, `BadgeVariant`, `BadgeProps`, `CardDetail`, `CardProps`, `KeyValueListItem`, `KeyValueListProps`, `TableProps`, `TableCaptionProps`, `TableHeaderProps`, `TableBodyProps`, `TableFooterProps`, `TableRowProps`, `TableHeadProps`, `TableCellProps`, `TableCellValue`, `TableColumn`, `TableRowData`, `TableSort`, `DataTableProps`, `DateRange`, `DateRangePickerProps`, `OverlayAction`, `OverlayTrigger`, `DrawerProps`, `FormFieldProps`, `FocusHandle`, `InputProps`, `KbdProps`, `LineChartPoint`, `LineChartSeries`, `LineChartTone`, `LineChartProps`, `SingleCalendarProps`, `RangeCalendarProps`, `CalendarProps`, `PopoverProps`, `SelectOption`, `SelectProps`, `TabItem`, `TabsProps`, `TextProps`, `ToolbarItem`, `ToolbarProps`, `NavigationItemProps`, `NotifyType`, `NotifyOptions`, `AsyncNotifyOptions`.

Shadcn provenance: Table maps to shadcn Table; AlertDialog maps to shadcn Alert Dialog; ActionMenu maps to shadcn Dropdown Menu; Toast maps to the current shadcn Toast backed by Base UI Toast. Toolbar is an approved exception: shadcn has no equivalent for a fixed, controlled contextual-action surface with animated presence and layout. These contracts intentionally do not expose raw primitives, class names, render props, slots, or styling overrides.

`globals.css` is the only non-TypeScript export. It supplies the UI-owned theme and component styling.

Removed public surfaces: Label; Separator; KbdGroup; Select parts; Tabs parts;
Drawer portal/backdrop/swipe handle and
